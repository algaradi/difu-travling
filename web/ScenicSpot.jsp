...
<table class="hovertable" id="hotel">
    ...
    <c:forEach items="${scenicSpots}" var="s">
        <tr οnmοuseοver="this.style.backgroundColor='#ffff66';" οnmοuseοut="this.style.backgroundColor='#d4e3e5';">
            ...
            <td>${s.photo}</td>
            <td>${s.descSpot}</td>
            <td><a href="scenic_modify.jsp?spotName=${s.spotName}&price=${s.price}&cityName=${s.cityName}&address=${s.address}&ticketNum=${s.ticketNum}&photo=${s.photo}&descSpot=${s.descSpot}">
                <button class="button button1">编辑</button>
            </a></td>
            <td>
                <a href="JavaScript:;">
                    <button class="button button3"  onclick="dodele('${s.spotName}')" >删除</button>
                </a>
            </td>
        </tr>
    </c:forEach>
